...
#ikm = b"********************************"   # non-random for now.
#SK = blst.Scalar().from_bendian(ikm)
print_bytes("SK", SK.to_bendian())
# Signer's public key
PK = blst.P1(SK).to_affine()
print_bytes("PK", PK.serialize())

# User wants to have |msg| signed, chooses random |r|,
#r = blst.Scalar().from_bendian(os.urandom(32))
#r = blst.Scalar().from_bendian(b"yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy")
#r = blst.Scalar(b"yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy");
r = blst.Scalar(b"11111111111111111111111111111111");
print_bytes("r", r.to_bendian());
# blinds the H(|msg|) with |r| and sends it to the Signer.
#hash = blst.P2().hash_to(msg, DST);
#print_bytes("hash", hash.serialize());

sig_for_wire = blst.P2().hash_to(msg, DST).sign_with(r).serialize()
print_bytes("sig_for_wire", sig_for_wire)

# Signer signs and sends the result back to the User.
sig_for_wire = blst.P2(sig_for_wire).sign_with(SK).serialize()
print_bytes("mint_sig_for_wire: ", sig_for_wire)
...
print_bytes("signature_inverse", signature.serialize())

# and now it can be verified as following...
print_bytes("msg", msg)
ctx = blst.Pairing(True, DST)
ctx.aggregate(PK, signature, msg, b"")
ctx.commit()
if not ctx.finalverify():
    raise AssertionError("disaster")

print("OK -- blind sig verified")
